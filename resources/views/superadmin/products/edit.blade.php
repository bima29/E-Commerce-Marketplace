...
@section('content')
    @php
        $updateAction = \Illuminate\Support\Facades\Route::has('superadmin.products.update')
            ? route('superadmin.products.update', ['product' => $product->id])
            : url('/superadmin/products/' . ($product->id ?? 0));

        $backHref = \Illuminate\Support\Facades\Route::has('superadmin.products.index')
    ...
    <div class="rounded-2xl bg-gradient-to-r from-primary-900 via-primary-800 to-primary-700 p-6 md:p-8 mb-6 shadow-xl">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-sm text-primary-100">Kelola produk per seller.</p>
            </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        <form method="POST" action="{{ $updateAction }}" class="space-y-5">
            ...
            <div>
                ...
                <input name="name" value="{{ old('name', $product->name ?? '') }}" class="mt-2 w-full rounded-lg border-gray-300 focus:border-primary-900 focus:ring-primary-900" placeholder="Contoh: Kaos Polos" />
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700">Seller</label>
                <select name="seller_id" class="mt-2 w-full rounded-lg border-gray-300 focus:border-primary-900 focus:ring-primary-900">
                    ...
                    @foreach (($sellers ?? []) as $seller)
                        <option value="{{ $seller->id }}" {{ (string)old('seller_id', $product->seller_id ?? '') === (string)$seller->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="price" value="{{ old('price', $product->price ?? '') }}" class="mt-2 w-full rounded-lg border-gray-300 focus:border-primary-900 focus:ring-primary-900" placeholder="10000" />
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700">Stok</label>
                    <input type="number" name="stock" value="{{ old('stock', $product->stock ?? '') }}" class="mt-2 w-full rounded-lg border-gray-300 focus:border-primary-900 focus:ring-primary-900" placeholder="0" />
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="4" class="mt-2 w-full rounded-lg border-gray-300 focus:border-primary-900 focus:ring-primary-900" placeholder="Deskripsi singkat produk">{{ old('description', $product->description ?? '') }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700">Image URL</label>
                <input name="image_url" value="{{ old('image_url', $product->image_url ?? '') }}" class="mt-2 w-full rounded-lg border-gray-300 focus:border-primary-900 focus:ring-primary-900" placeholder="https://..." />
            </div>
