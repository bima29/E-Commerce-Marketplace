...
@section('content')
@php
    $postAction = \Illuminate\Support\Facades\Route::has('superadmin.login.submit')
        ? route('superadmin.login.submit')
...
<div class="container mx-auto px-4 lg:px-8 py-10">
    <div class="max-w-md mx-auto bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            <h1 class="text-xl font-bold text-gray-900">Login Superadmin</h1>
            <p class="text-sm text-gray-600 mt-1">Masuk untuk mengelola seller & produk.</p>
        </div>
        ...
        <form method="POST" action="{{ $postAction }}" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" class="w-full rounded-lg border border-gray-300 px-3 py-3 focus:border-primary-900 focus:ring-primary-900" placeholder="admin@contoh.com" />
            </div>
            ...
            <div class="text-xs text-gray-500">
                Gunakan akun seed dari README (role superadmin).
            </div>
        </form>
    </div>
</div>
